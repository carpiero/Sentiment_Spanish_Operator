...
def update_twitter_database():
    ...
    auth = tweepy.OAuthHandler(CONSUMER_KEY, CONSUMER_SECRET)
    auth.set_access_token(ACCESS_TOKEN, ACCESS_SECRET)
    api2 = tweepy.API(auth,wait_on_rate_limit=True,wait_on_rate_limit_notify=True,retry_count=5,retry_delay=5)
    ...
    auth = tweepy.OAuthHandler(CONSUMER_KEY, CONSUMER_SECRET)
    auth.set_access_token(ACCESS_TOKEN, ACCESS_SECRET)
    api3 = tweepy.API(auth,wait_on_rate_limit=True,wait_on_rate_limit_notify=True,retry_count=5,retry_delay=5)
    ...
    tweets_df_total['entities'] = tweets_df_total['entities'].apply(literal_eval)
    tweets_df_total = tweets_df_total.drop(columns=['API'])

    tweets_df_total['urls'] = tweets_df_total['entities'].apply(urls)
    ...
    list_tweet = list(tweets_df_total['Tweet_Content'])

    print(f'\n\n Clasificando tweets')

    cl = list_tweet
    jump = 2000
    cla = [classifier(cl[x:x + jump]) for x in range(0 , len(cl) , jump)]
    cla = [y for x in cla for y in x]

    cl_label = [rem_stars(x.get('label')) for x in cla]
    cl_score = [x.get('score') for x in cla]

    tweets_df_total['stars'] = pd.Series((cl_label) , index=tweets_df_total.index)
    tweets_df_total['stars_score'] = pd.Series((cl_score) , index=tweets_df_total.index)

    tweets_df_total['entities'] = tweets_df_total['entities'].apply(json.dumps)
